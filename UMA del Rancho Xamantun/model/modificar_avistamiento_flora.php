...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        if (isset($_FILES['input_imagen']) && $_FILES['input_imagen']['error'] === UPLOAD_ERR_OK) {
            ...
            $stmt = $conn->prepare("SELECT ruta_imagen FROM avistamientos_flora WHERE id_avistamiento = :id_avistamiento");
        ...
        $sql = "UPDATE avistamientos_flora SET id_avistamiento_especie=:id_avistamiento_especie, 
        ...
        header('Location: ../avistamiento_floraR.php');
    } catch (PDOException $e) {
        echo "Ha ocurrido un error: " . $e->getMessage();
    }
} else {
    // Consulta SQL preparada para la inserción
    $sql2 = "SELECT * FROM avistamientos_flora WHERE id_avistamiento = :id";
